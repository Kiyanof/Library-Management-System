...
final public class Persian extends Base{
    ...
    @Override
    protected boolean setYear(int year) {
        int currentYear = getCurrentYear();
        try {
            if ((!canFuture() && year >= 1350 && year <= currentYear) ||
                    (canFuture() && year >= currentYear)){
                this.year = year;
                return true;
            }
            return false;
    ...
    @Override
    protected boolean setMonth(int month) {
        ...
        try {
            if (month >= 1 && month <= 12){
                if (((!futureCondition) && (year != currentYear || month <= currentMonth)) ||
                        (futureCondition && month >= currentMonth && year == currentYear) ||
                        (futureCondition && year > currentYear)){
                    this.month = month;
                    return true;
                }
                System.out.print("\n--- WARNING: Please enter valid month value to this year.");
                return false;
            }
            System.out.print("\n--- WARNING: Please enter the month between 1 to 12.");
            return false;
        } catch (Exception e){
            System.out.print("\n--- ERROR:" + e.getMessage() + "!!!");
            return false;
    ...
    @Override
    protected boolean setDay(int day) {
        int year = this.getYear();
            ...
            if (day >= 1 && day <= 31) {
                if (canFuture()){ // NOTE
                    ...
                    if (year == currentYear && month == currentMonth && day <= currentDay){
                        this.day = day;
                        return true;
                    } else if (year == currentYear && month < currentMonth && isValidDay(year, month, day)){
                        this.day = day;
                        return true;
                    } else if (year < currentYear && isValidDay(year, month, day)){
                        this.day = day;
                        return true;
                    }
                }
            }
            System.out.print("\n--- WARNING: Please enter the valid day value.");
            return false;
        } catch (Exception e){
            System.out.print("\n--- ERROR:" + e.getMessage() + "!!!");
            return false;
